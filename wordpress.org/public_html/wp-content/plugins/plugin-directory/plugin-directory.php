 ...
 * and the release becoming the identity recorded in `update_source` for the
 * api.wordpress.org plugin update-check API. Any prior identity stays recorded until
 * the cooldown elapses. Mitigates supply-chain attacks by giving scanners and humans
 * a window to flag bad releases. Plugin reviewers can bypass the cooldown via the
 * wp-admin force-release action; see Jobs\API_Update_Updater::force_release().
 *
 ...
 *
 * Defaults to 0 (cooldown disabled, release identities written immediately) for now; this will be
 ...
 * @return int Delay in seconds. 0 writes the release identity without a cooldown.
 */
function get_release_cooldown_delay( $plugin_slug = '' ) {
	/**
	 * Filters the release cooldown delay for a plugin.
	 *
	 * Return 0 to write the release identity as soon as it is imported, or
