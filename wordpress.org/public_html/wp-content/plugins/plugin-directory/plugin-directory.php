 ...
 * and the release being written to the `update_source` table — and so served to
 * sites by the api.wordpress.org plugin update-check API. The previous release remains
 ...
 * via the wp-admin force-release action; see Jobs\API_Update_Updater::force_release().
 *
 ...
 * @return int Delay in seconds. 0 disables the cooldown (the release is served immediately).
 */
function get_release_cooldown_delay( $plugin_slug = '' ) {
	/**
	 * Filters the release cooldown delay for a plugin.
	 *
	 * Return 0 to disable the cooldown (the release is served as soon as it is imported), or
