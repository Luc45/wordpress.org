...
/**
 * Tests that completed security scan callbacks block high-risk releases pending review.
 *
...
class Security_Scan_Block_Test extends TestCase {
	...
	/**
	 * Deliver a callback with the digest of its exact test body.
	 *
	 * @param array $callback Callback data.
	 * @return true|\WP_Error Callback result.
	 */
	private function handle_callback( array $callback ) {
		$body = wp_json_encode( $callback );
		$this->assertIsString( $body );

		return Plugin_Scan_Gandalf::handle_callback(
			get_post( $this->plugin->ID ),
			$callback,
			hash( 'sha256', $body )
		);
	}
	...
	public function test_high_risk_scan_blocks_release(): void {
		...
		$result = $this->handle_callback( $this->completed_callback() );
	...
	public function test_block_leaves_findings_note(): void {
		...
		$this->handle_callback( $this->completed_callback() );
		...
		$this->assertStringContainsString( 'Release ref ' . self::VERSION . ' (' . self::VERSION . ') is blocked', $note );
	...
	public function test_threshold_boundary_blocks(): void {
		...
		$this->assertTrue( $this->handle_callback( $callback ) );
		$this->assertTrue( API_Update_Updater::is_release_blocked( $this->get_release() ) );
	}
	...
	/**
	 * A high-risk result is not consumed as advisory when its release is missing.
	 */
	public function test_high_risk_scan_remains_pending_when_block_fails(): void {
		$result = $this->handle_callback( $this->completed_callback() );

		$this->assertInstanceOf( \WP_Error::class, $result );
		$this->assertSame( 'security_scan_block_failed', $result->get_error_code() );
		$this->assertEmpty( get_post_meta( $this->plugin->ID, Plugin_Scan_Gandalf::LAST_RESULT_META_KEY, true ) );
	...
	/**
	 * Corrupt persisted scan state is not reinterpreted as an empty initial map.
	 */
	public function test_empty_string_pending_state_throws(): void {
		update_post_meta( $this->plugin->ID, Plugin_Scan_Gandalf::PENDING_META_KEY, '' );

		$this->expectException( \UnexpectedValueException::class );
		$this->expectExceptionMessage( 'metadata is not a map' );
	...
	public function test_high_install_count_still_blocks(): void {
		...
		$this->assertTrue( $this->handle_callback( $this->completed_callback() ) );
		$this->assertTrue( API_Update_Updater::is_release_blocked( $this->get_release() ) );
	}

	/**
	 * A verdict for an already-served release burns it without pretending the
	 * existing update row was un-shipped.
	 */
	public function test_served_version_is_burned_without_unshipping(): void {
		...
		$this->assertTrue( $this->handle_callback( $this->completed_callback() ) );
		$this->assertTrue( API_Update_Updater::is_release_blocked( $this->get_release() ) );
		$this->assert_served_release( self::VERSION, self::VERSION );
	...
	/**
	 * A verdict burns its exact release even after a newer release supersedes it.
	 */
	public function test_superseded_version_is_burned(): void {
		$this->stage_release();
	...
	public function test_conflicting_replay_is_rejected(): void {
		...
		$result      = $this->handle_callback( $conflicting );
	...
	/**
	 * A re-marshalled callback is not an exact-body retry.
	 */
	public function test_reordered_replay_is_rejected(): void {
		...
		$result = $this->handle_callback( $reordered );
		$this->assertInstanceOf( \WP_Error::class, $result );
		$this->assertSame( 'security_scan_conflict', $result->get_error_code() );
		$this->assertCount( 1, $this->get_internal_notes() );
	}
	...
	/**
	 * A failed callback is terminal, so a later completed body conflicts.
	 */
	public function test_completed_verdict_conflicts_with_failed_report(): void {
		$this->stage_release();

		$this->assertTrue( $this->handle_callback( $this->failed_callback() ) );
	...
	public function test_mismatched_version_is_rejected(): void {
		$result = $this->handle_callback( $this->completed_callback( array( 'version' => '9.9.9' ) ) );
	...
	public function test_block_holds_update_source_on_served_version(): void {
		...
		$this->assert_served_release( '1.0.0', '1.0.0' );
	}
