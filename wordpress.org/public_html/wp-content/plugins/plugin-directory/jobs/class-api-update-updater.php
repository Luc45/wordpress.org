...
class API_Update_Updater {
	...
	public static function update_single_plugin( $plugin_slug ) {
		...
		$stable_tag       = get_post_meta( $post->ID, 'stable_tag', true ) ?: 'trunk';
		$requires_plugins = get_post_meta( $post->ID, 'requires_plugins', true );
		$release_key      = 'trunk' === $stable_tag ? "trunk@{$version}" : $stable_tag;
		$release          = Plugin_Directory::get_release_by_tag( $post, $release_key );
